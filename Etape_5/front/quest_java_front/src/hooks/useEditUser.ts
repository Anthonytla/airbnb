...
const useEditUser = () => {
  ...
  const userEditMutation = useMutation((formData: AuthUser) => UserService.edit(userId, formData), {
    onMutate: async (formData: AuthUser) => {
      await queryClient.cancelQueries(['user', userId]);
      const previousUserData = queryClient.getQueryData(['user', userId]) as User;
